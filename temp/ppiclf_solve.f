...
! 
     ...
     >                 +rnz*ppiclf_cp_map(3,i)+rd)
            ...
            ! give a little extra room for walls (20x)
            if (rdist .gt. 20.0d0*ppiclf_d2chk(3)) goto 1511
...
! Internal: 
! 
      integer*4 i, ndum, nstage, istage,ii
      integer beginning,rate,end
!
! Output:
!
...
!
      ...
      call ppiclf_solve_SetYdot(1)
      do i=1,ppiclf_npart
         if (ABS(ppiclf_y(PPICLF_JVX,i) - 18.555555) .lt. 1E-5) then
               ppiclf_y(PPICLF_JVX,i) = ppiclf_rprop(PPICLF_R_JUX,i)
               ppiclf_y(PPICLF_JVY,i) = ppiclf_rprop(PPICLF_R_JUY,i)
               ppiclf_y(PPICLF_JVZ,i) = ppiclf_rprop(PPICLF_R_JUZ,i)
         endif
      ...
      do istage=1,nstage
         ! evaluate ydot
         call ppiclf_solve_SetYdot(istage)
         ! rk3 integrate
         do i=1,ndum
            ndum = PPICLF_NPART*PPICLF_LRS
     ...
     >                     + ppiclf_rk3coef(3,istage)*ppiclf_ydot (i,1)
      ...
      do ii=1,ppiclf_npart
            call ppiclf_solve_NearestNeighbor(ii)
      enddo

      do i=1,ppiclf_npart
         if (ABS(ppiclf_y(PPICLF_JVX,i)) .gt. 30) then
               print*, "ppiclf_nid", ppiclf_nid, "i:", i
               print*, "ERROR", ppiclf_y(PPICLF_JVX,i)
         endif
...
c----------------------------------------------------------------------
      subroutine ppiclf_solve_SetYdot(arg)
!
      implicit none
!
      include "PPICLF"
!
      integer beginning,rate,end,arg

      call ppiclf_solve_InitSolve(arg)
...
c----------------------------------------------------------------------
      subroutine ppiclf_solve_InitSolve(arg)
!
      implicit none
!
      include "PPICLF"
...
!      print*, "arg:", arg
      if (ppiclf_lproj .and. ppiclf_overlap .and. (arg .gt. 2)) 
...
!
     ...
     >              +rnz*rx2(3)+rd)
         ...
         if (1 .eq. -1) then
            ...
            print*, "ppiclf_wall_c(1,j)", ppiclf_wall_c(1,j)
            print*, "ppiclf_wall_c(2,j)", ppiclf_wall_c(2,j)
            print*, "ppiclf_wall_c(3,j)", ppiclf_wall_c(3,j)
            print*, "ppiclf_wall_c(is+1,j)", ppiclf_wall_c(istride+1,j)
            print*, "ppiclf_wall_c(is+2,j)", ppiclf_wall_c(istride+2,j)
            print*, "ppiclf_wall_c(is+3,j)", ppiclf_wall_c(istride+3,j)

            print*, "ppiclf_ndim", ppiclf_ndim
            print*, "wall j:", j
            print*, "rdist:", rdist
            print*, "ppiclf_d2chk(2):", ppiclf_d2chk(2)
         endif
